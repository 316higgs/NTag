...
export card=$soft/skdetsim-skgd/sample.card #SKDETSIM-SKGD CARD commented 020221
#export card=$soft/skdetsim-skgd/trunk/sk4_odtune.card #SKDTESIM-SKGD_NEW CARD
#export card=$soft/skdetsim-skgd/branches/merge_ANNRIGd/sk5_odtune.card #merge_ANNRIGd card
#export card=$soft/skdetsim-skgd/sk5_odtune.card #SKDETSIM-SKGD CARD

#export card=$soft/skdetsim-skgd-annri-backup/sample.card #old working
...
if $DO_DETSIM; then
    ...
    export G4WORKDIR=$soft/skdetsim-skgd/geant4_work/ #comment 020221
#    export G4WORKDIR=$soft/skdetsim-skgd-annri-backup/geant4_work/
    #export G4WORKDIR=$soft/trunk/skdetsim-skgd/geant4_work/
    #export G4WORKDIR=$soft/skdetsim-skgd/branches/merge_ANNRIGd/geant4_work/
...
#    export G4NEUTRONHP_USE_ONLY_PHOTONEVAPORATION=1
    ...
    cd $soft/skdetsim-skgd/ #commented 020221
    #cd $soft/skdetsim-skgd/branches/merge_ANNRIGd/

#    cd $soft/skdetsim-skgd-annri-backup/
    echo "$soft/skdetsim-skgd/skdetsim $card $((num)) $f_ds_vec $f_ds_out $f_ds_hbk" | tee -a  $log
    echo "$soft/skdetsim-skgd/trunk/skdetsim $card $((num)) $f_ds_vec $f_ds_out $f_ds_hbk" | tee -a  $log
   ...
   # $soft/skdetsim-skgd-annri-backup/skdetsim $card $f_ds_out $f_ds_vec         | tee -a  $log
    ##$soft/skdetsim-skgd/trunk/skdetsim $card $f_ds_out $f_ds_vec         | tee -a  $log #SKDETSIM SK-GD #standard skdetsim I/O
    #$soft/skdetsim-skgd/branches/merge_ANNRIGd/skdetsim $card $f_ds_out $f_ds_vec | tee -a $log #SKDETSIM SK-GD merge_ANNRIGd I/O
    echo | tee -a  $log
...
#    cp -v $f_lf_out $f_lf_dir    | tee -a  $log #comment out to make lowfit files appear
#else
...
export f_tag_dir=$odir/ntag
export f_tag_in=$f_ntp_out
export f_tag_out=ntag_$id.root
export f_tag_bkg=nbkg_ntag_$id.root
export f_tag_sig=nsig_ntag_$id.root

if $DO_NTAG; then
    [ ! -e $f_tag_in ] && echo "Missing input file $f_tag_in, bailing" && exit 3
    [ ! -d $f_tag_dir ] && mkdir -p $f_tag_dir

#    exe=/disk02/usr6/fiacob/ncneut/ntag_mc/ntag_mc
    exe=/disk02/usr6/pmehta/ntag_mc/ntag_mc
    echo $RFLIST

    $exe $f_tag_in $f_tag_out      | tee -a  $log

    echo                                    | tee -a  $log
    cp -v $f_tag_out $f_tag_dir             | tee -a  $log
    cp -v $f_tag_sig $f_tag_dir             | tee -a  $log
    cp -v $f_tag_bkg $f_tag_dir             | tee -a  $log
else
    # Commented 17/06/2019
    #echo "Restoring previous ntuple"        | tee -a  $log
    #cp -v $f_ntp_dir/$f_ntp_out .           | tee -a  $log
...
echo                       | tee -a  $log
echo "===================" | tee -a  $log
echo "==== Sectuple ====" | tee -a  $log
echo "===================" | tee -a  $log
date                       | tee -a  $log

export f_stp_dir=$odir/sectuple
export f_stp_intag=$f_tag_dir/$f_tag_out
export f_stp_insig=$f_tag_dir/$f_tag_sig
export f_stp_inbkg=$f_tag_dir/$f_tag_bkg
export f_stp_ontag=seclentp_$id.root
export f_stp_onsig=nsig_seclentp_$id.root
export f_stp_onbkg=nbkg_seclentp_$id.root

if   [ "$mode" == "fhc" ]; then
#    export MLPTHRESHOLD=0.8536019286
#    export MLPTHRESHOLD=0.7852655170 #working point after Gd bug fix (Gd and H)
#    export MLPTHRESHOLD=0.8385330357 #working point for h capture only
#     export MLPTHRESHOLD=0.8122121676 #working point for Gd capture only
     export MLPTHRESHOLD=0.5066889799
elif [ "$mode" == "rhc" ]; then
#    export MLPTHRESHOLD=0.8536019286
    #    export MLPTHRESHOLD=0.7852655170
#    export MLPTHRESHOLD=0.8385330357
    export MLPTHRESHOLD=0.5066889799
fi


if $DO_SECTUPLE; then
    [ ! -e $f_stp_in ] && echo "Missing input file $f_stp_in, bailing" && exit 3
    [ ! -d $f_stp_dir ] && mkdir -p $f_tag_dir

    exe=/disk02/usr6/pmehta/ncqeana/tools/applyclassifier/applyclassifier
    echo $RFLIST
    
    $exe $f_stp_intag $f_stp_insig $f_stp_inbkg $f_stp_ontag $f_stp_onsig $f_stp_onbkg $MLPTHRESHOLD   | tee -a  $log

    echo                                    | tee -a  $log
    cp -v $f_stp_ontag $f_stp_dir             | tee -a  $log
    cp -v $f_stp_onsig $f_stp_dir             | tee -a  $log
    cp -v $f_stp_onbkg $f_stp_dir             | tee -a  $log
else
    # Commented 17/06/2019
    #echo "Restoring previous ntuple"        | tee -a  $log
    #cp -v $f_ntp_dir/$f_ntp_out .           | tee -a  $log
