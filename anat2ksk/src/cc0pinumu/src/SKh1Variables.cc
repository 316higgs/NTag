...
//	t->SetBranchStatus("*", 0);
	...
	for(Int_t i=0; i<(Int_t)fVarList.size(); i++)
	{
		...
		else if( fVarList[i]=="fq1rt0" ) 			{ t->SetBranchAddress(fVarList[i].c_str(),	ffq1rt0); }
		...
		else if( fVarList[i]=="fqn50" ) 			{ t->SetBranchAddress(fVarList[i].c_str(),	ffqn50); }
...
void SKh1Variables::resisterVariable(string var)
{
	...
	else if( var=="fq1rt0" )			{ fVarList.push_back( var ); }
	...
	else if( var=="fqn50" )				{ fVarList.push_back( var ); }
...
template <typename T> T SKh1Variables::var(string var, int i, int j) const
{
	...
	else if( var=="fq1rt0" )	{ t=ffq1rt0[i][j]; }
