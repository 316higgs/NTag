...
void NtupleMaker::SetAmbePosition( const char *ambepos ) {
	...
	if ( ambepos == NULL ) {
		...
		ambe_pos[1] = -70.7;
		ambe_pos[2] = 0.0;
		cerr << "AMBE CENTRAL MC " << ambe_pos[0] << " " << ambe_pos[1] << " " << ambe_pos[2] << endl;
	}
		...
		ambe_pos[1] = -70.7;
		ambe_pos[2] = 1500.0;
		cerr << "AMBE TOP MC " << ambe_pos[0] << " " << ambe_pos[1] << " " << ambe_pos[2] << endl;
	}
		...
		ambe_pos[1] = -1201.9;
		ambe_pos[2] = 0.0;
		cerr << "AMBE BARREL MC " << ambe_pos[0] << " " << ambe_pos[1] << " " << ambe_pos[2] << endl;
	}
	else {
...
//Added by Seidai(2021.7.12)
void NtupleMaker::prompt_hit_veto(int hitnum, float hittime) {
  std::cout << " " << std::endl;
  std::cout << "[##### NtupleMaker::prompt_hit_veto #####]  PMT hit#[" << hitnum << "]  PMT Hit Time exceeds 1 microsec: " << hittime/1000 << " microsec  <------ Start Filling" << std::endl;
}
...
bool NtupleMaker::ReadMCEvent() {
    ...
    */
    ...
    bool neutrontime = false; //veto prompt hits
    int neutrontime_bigin = 0;
    ncg.nqiskz = 0;
    std::cout << "[##### NtupleMaker::ReadMCEvent #####]  sktqz_.nqiskz = " << sktqz_.nqiskz << std::endl;
    for (int i=0; i<sktqz_.nqiskz; i++) {
        ...
        {
            //record data after 1 microsec
            //static std::once_flag flag;
            //static bool neutrontime_flag = true;
            bool neutrontime_flag = true;
            //if(sktqz_.tiskz[i]/1000>10) neutrontime = true; //PMT hit timing cut (>10 microsec)
            //if(sktqz_.tiskz[i]/1000>1) neutrontime = true; //PMT hit timing cut (>1 microsec)
            if(sktqz_.tiskz[i]/1000>5) neutrontime = true; //PMT hit timing cut (>5 microsec)
            //if(sktqz_.tiskz[i]/1000>18) neutrontime = true; //PMT hit timing cut (>18 microsec)
            if(neutrontime) {
                //Show at once
                if (neutrontime_flag) {
                    //prompt_hit_veto(i, sktqz_.tiskz[i]);
                    neutrontime_bigin = i; //record the starting event
                    neutrontime_flag = false;
                }
                /*std::cout << "[##### NtupleMaker::ReadMCEvent #####]  ";
                std::call_once(flag, prompt_hit_veto, i, sktqz_.tiskz[i]);
                std::cout << "  <---- Start Filling" << std::endl;*/
                ncg.icabiz[ncg.nqiskz] = sktqz_.icabiz[i];
                ncg.tiskz[ncg.nqiskz]  = sktqz_.tiskz[i]; //unit is nsec
                ncg.qiskz[ncg.nqiskz]  = sktqz_.qiskz[i];
                std::cout << "[##### NtupleMaker::ReadMCEvent #####]  ncg.icabiz[" << ncg.nqiskz << "] = " << ncg.icabiz[ncg.nqiskz] << ", ncg.tiskz[" << ncg.nqiskz << "] = " << ncg.tiskz[ncg.nqiskz] << ", ncg.qiskz[" << ncg.nqiskz << "] = " << ncg.qiskz[ncg.nqiskz] << std::endl;
                ++ncg.nqiskz;
            }   
        }
    }
    std::cout << "[##### NtupleMaker::ReadMCEvent #####]  ncg.nqiskz = " << ncg.nqiskz << ", Loss: sktqz_.nqiskz - ncg.nqiskz = " << sktqz_.nqiskz - ncg.nqiskz << std::endl;
    //std::cout << "[##### NtupleMaker::ReadMCEvent #####]  Detector simulation information" << std::endl;
    //watch(sktqz_.nqiskz);
    //watch(ncg.nqiskz);
    //watch(sktqz_.nqiskz - ncg.nqiskz);
 
    ncg.tbgn = ncg.tiskz[0];            //begining of PMT hit (prompt hit is removed)
    ncg.tend = ncg.tiskz[ncg.nqiskz-1]; //end of PMT hit
    ncg.tdur = ncg.tend - ncg.tbgn;     //duration of PMT hits
    watch(ncg.tbgn);
    watch(ncg.tend);
    watch(ncg.tdur);
    //return true;
    ...
    //If exist BONSAI information --------------------------------------------------------------------------------------------------
    std::cout << "zbsget.rdata[29](before kzget1_()): " << zbsget.rdata[29] << std::endl;
    kzget1_("LOWBS3", isegm, ndata, zbsget.idata, 6);
    std::cout << "zbsget.rdata[29](after kzget1_()) : " << zbsget.rdata[29] << std::endl;
    //If BONSAI reco energy is larger than 0.0001, record it
    if (zbsget.rdata[29] > 0.0001) {
        if (verbose) std::cout << "[##### NtupleMaker::ReadMCEvent #####]  Loading Bonsai3 fit information" << std::endl;
    ...
    //double bsovaq = (bsgoodness*bsgoodness)-(bsdirks*bsdirks);  //         <--------------------------- bonsai

    // Call fiducial variables
    int idod=1; //from id wall
    double poswal[3]={0,0,0};
    double effwall = effwallf(idod, bonsai_vertex, bonsai_dir, poswal); // <--------------------------- bonsai
    ...
    ncg.n50   = bsn50;      // N50 from Bonasi fit    <--------------------------- bonsai

    for (int i=0; i<3; i++) {               //        <--------------------------- bonsai
        ncg.pos[i] = bonsai_vertex[i];      //        <--------------------------- bonsai
        ncg.bdir[i] = bonsai_dir[i];        //        <--------------------------- bonsai
		// Force position for Am/Be
		if ( is_ambe ) {
			ncg.pos[i] = ambe_pos[i];
		}
    }
    //Fill BONSAI variables
    //ncg.erec     = bsenergy;                //        <--------------------------- bonsai
    ncg.wall     = wallsk(bonsai_vertex);   //        <--------------------------- bonsai
    ncg.effwall  = effwall;
    //ncg.ovaq     = bsovaq;                  //        <--------------------------- bonsai
    CherenkovAngle(bonsai_vertex, true);    //        <--------------------------- bonsai
    CherenkovAngle(bonsai_vertex, false);   //        <--------------------------- bonsai
    //ncg.goodness = bsgoodness;              //        <--------------------------- bonsai

    // search decay-e for post-activity cut
    int muerr = 1;
    float vertex_mue[ 3 ];
     vertex_mue[0] = (float)bonsai_vertex[ 0 ]; //        <--------------------------- bonsai
     vertex_mue[1] = (float)bonsai_vertex[ 1 ]; //        <--------------------------- bonsai
     vertex_mue[2] = (float)bonsai_vertex[ 2 ]; //        <--------------------------- bonsai
    ...
    */
    ...
    ///*
    //------------------------------------------   fiTQun   -------------------------------------
    // Get fiTQun information.
    double fitqun_vertex[3];  //double bonsai_vertex[3];
    double fitqun_dir[3];     //double bonsai_dir[3];
    int fitqun_n50;           //int    bsn50    = 0;
    double bsgoodness  = 0;
    double bsdirks  = 0;
    double bsenergy = 0;
    int ndata, isegm = 1;
    ...
    int fqbank = 0;
    bool fitqun_loaded = true; //bool bonsai_loaded = false;

    ///*
    //without PMT hit timing cut
    if(fitqun_loaded) std::cout << "[##### NtupleMaker::ReadMCEvent #####]  Loading fiTQun information" << std::endl;
    ...
    //std::cout << "[##### NtupleMaker::ReadMCEvent #####]  tiskz:" << 
    std::cout << "[##### NtupleMaker::ReadMCEvent #####]  Reco Vertex position :" << fitqun1r_.fqnse << " subevents, fq1rpos[0][2]=(" 
              << fitqun1r_.fq1rpos[0][2][0] << ", " << fitqun1r_.fq1rpos[0][2][1] << ", " << fitqun1r_.fq1rpos[0][2][2] << ")" << std::endl;
    std::cout << "[##### NtupleMaker::ReadMCEvent #####]  Reco direction :(" 
              << fitqun1r_.fq1rdir[0][2][0] << ", " << fitqun1r_.fq1rdir[0][2][1] << ", " << fitqun1r_.fq1rdir[0][2][2] << ")" << std::endl;
    //fiTQun variables
    fitqun_n50 = *(fitqun1r_.fqn50);
    for (int dim=0; dim<3; dim++) {
        fitqun_vertex[dim] = fitqun1r_.fq1rpos[0][2][dim];
        fitqun_dir[dim]    = fitqun1r_.fq1rdir[0][2][dim];
    }
    //std::cout << "fitqun_vertex[]=(" << fitqun_vertex[0] << ", " << fitqun_vertex[1] << ", " << fitqun_vertex[2] << ")" << std::endl;
    //std::cout << "fitqun_dir[]=(" << fitqun_dir[0] << ", " << fitqun_dir[1] << ", " << fitqun_dir[2] << ")" << std::endl;
    //if (!fitqun_loaded) std::cout << "[##### NtupleMaker::ReadMCEvent #####]  No fiTQun information." << std::endl;

    //lowbs3get_();

    // Reconstruction quality
    double bsovaq = (bsgoodness*bsgoodness)-(bsdirks*bsdirks);  //         <--------------------------- bonsai(try to comment out)

    // Call fiducial variables
    int idod=1; //from id wall
    double poswal[3]={0,0,0};
    double effwall = effwallf(idod, fitqun_vertex, fitqun_dir, poswal); //double effwall = effwallf(idod, bonsai_vertex, bonsai_dir, poswal); // <------- bonsai -> fiTQun
    ...
    ncg.potot = skq_.qismsk; //TQI->qismsk, total ID p.e.
    ncg.nhit  = skq_.nqisk;  // Total ID hits
    ncg.n50 = fitqun_n50;    //ncg.n50   = bsn50;      // N50 from Bonasi fit    <--------------------------- bonsai -> fiTQun

    //Fill fiTQun variables
    for (int i=0; i<3; i++) {               
        ncg.pos[i] = fitqun_vertex[i];      //ncg.pos[i] = bonsai_vertex[i];      //        <--------------------------- bonsai -> fiTQun
        ncg.bdir[i] = fitqun_dir[i];         //ncg.bdir[i] = bonsai_dir[i];        //        <--------------------------- bonsai -> fiTQun
        // Force position for Am/Be
        if ( is_ambe ) {
            ncg.pos[i] = ambe_pos[i];
        }
    }
    //std::cout << "ncg.pos[]=(" << ncg.pos[0] << ", " << ncg.pos[1] << ", " << ncg.pos[2] << ")" << std::endl;
    //std::cout << "ncg.bdir[]=(" << ncg.bdir[0] << ", " << ncg.bdir[1] << ", " << ncg.bdir[2] << ")" << std::endl;

    ncg.erec     = bsenergy;                //        <--------------------------- bonsai(try comment out)
    ncg.wall = wallsk(fitqun_vertex);         //ncg.wall = wallsk(bonsai_vertex);   //        <--------------------------- bonsai -> fiTQun
    ncg.effwall  = effwall;
    ncg.ovaq     = bsovaq;                  //        <--------------------------- bonsai(try comment out)
    /*
    std::cout << "[##### NtupleMaker::ReadMCEvent #####]  Call CherenkovAngle(fitqun_vertex, true)" << std::endl;
    CherenkovAngle(fitqun_vertex, true);      //CherenkovAngle(bonsai_vertex, true);    //        <--------------------------- bonsai -> fiTQun
    std::cout << "[##### NtupleMaker::ReadMCEvent #####]  Finish CherenkovAngle(fitqun_vertex, true)" << std::endl;
    std::cout << "----------------------------------------------------------------------------------" << std::endl;
    std::cout << "[##### NtupleMaker::ReadMCEvent #####]  Call CherenkovAngle(fitqun_vertex, false)" << std::endl;
    CherenkovAngle(fitqun_vertex, false);     //CherenkovAngle(bonsai_vertex, false);   //        <--------------------------- bonsai -> fiTQun
    std::cout << "[##### NtupleMaker::ReadMCEvent #####]  Finish CherenkovAngle(fitqun_vertex, false)" << std::endl;
    */
    //ncg.goodness = bsgoodness;              //        <--------------------------- bonsai(try comment out)
    //return true;

    // search decay-e for post-activity cut
    int muerr = 1;
    float vertex_mue[3];
    vertex_mue[0] = (float)fitqun_vertex[0];  //vertex_mue[0] = (float)bonsai_vertex[0]; //        <--------------------------- bonsai -> fiTQun
    vertex_mue[1] = (float)fitqun_vertex[1];  //vertex_mue[1] = (float)bonsai_vertex[1]; //        <--------------------------- bonsai -> fiTQun
    vertex_mue[2] = (float)fitqun_vertex[2];  //vertex_mue[2] = (float)bonsai_vertex[2]; //        <--------------------------- bonsai -> fiTQun
    //std::cout << "vertex_mue[]=(" << vertex_mue[0] << ", " << vertex_mue[1] << ", " << vertex_mue[2] << ")" << std::endl;
    muechk_(vertex_mue, &muerr);
    ncg.nmue = apmue_.apnmue; 
    //*/
    ...
    //If inpmt_() returns nonzero value, this data does not fill.
    int inpmt = -999999;
    inPMT = true;
    inpmt_(ncg.posv, &inpmt);
    std::cout << "[##### NtupleMaker::ReadMCEvent #####]  inpmt_() response: " << inpmt << std::endl;
    if(inpmt!=0) {
        std::cout << "[##### NtupleMaker::ReadMCEvent #####]  NOT Fill" << std::endl;
        inPMT = false;
    }
    //return true; //X
    ...
    for ( int isp = 0; isp < secndprc_.nscndprtc; ++isp ) {
        ...
        ncg.iprntprt[isp] = secndprc_.iprntprtc[isp];  // commented in old mc
        ncg.lmecscnd[isp] = secndprc_.lmecscndc[isp];
        ncg.iprnttrk[isp] = secndprc_.iprnttrkc[isp];  // commented in old mc
        ncg.iorgprt[isp] = secndprc_.iorgprtc[isp];  // commented in old mc
        ncg.pprnt[isp][0] = secndprc_.pprntc[isp][0];  // commented in old mc
        ncg.pprnt[isp][1] = secndprc_.pprntc[isp][1];  // commented in old mc
        ncg.pprnt[isp][2] = secndprc_.pprntc[isp][2];  // commented in old mc
        ncg.iflgscnd[isp] = secndprc_.iflgscndc[isp];  // commented in old mc
        ncg.pprntinit[isp][0] = secndprc_.pprntinitc[isp][0];  // commented in old mc
        ncg.pprntinit[isp][1] = secndprc_.pprntinitc[isp][1];  // commented in old mc
        ncg.pprntinit[isp][2] = secndprc_.pprntinitc[isp][2];  // commented in old mc
        ncg.vtxprnt[isp][0] = secndprc_.vtxprntc[isp][0];  // commented in old mc
        ncg.vtxprnt[isp][1] = secndprc_.vtxprntc[isp][1];  // commented in old mc
        ncg.vtxprnt[isp][2] = secndprc_.vtxprntc[isp][2];  // commented in old mc
        ncg.iprntidx[isp] = secndprc_.iprntidxc[isp];  // commented in old mc
        ncg.nchilds[isp] = secndprc_.nchildsc[isp];  // commented in old mc
        ncg.ichildidx[isp] = secndprc_.ichildidxc[isp];  // commented in old mc
    }
...
bool NtupleMaker::ReadDataEvent() {
    ...
    */
    ...
    if (LOWE->bseffhit[11] > 0.0001) {
        if (verbose) std::cout << "[##### NtupleMaker::ReadDataEvent #####]  Loading Bonsai3 fit information" << std::endl;
    ...
    */
    ...
    int fqbank = 0;
    bool fitqun_loaded = false; //bool bonsai_loaded = false;
    int ndata, isegm = 1;

    std::cout << "[##### NtupleMaker::ReadDataEvent #####]  Loading fiTQun information" << std::endl;
    readfqzbsbank_(&fqbank);
    ...
    //  - j: particle hypothesis -> 2 is for muon
    //  - k: special index (e.g. 0 for x)
    std::cout << "[##### NtupleMaker::ReadDataEvent #####]  Reco Vertex position :" << fitqun1r_.fqnse << " subevents, fq1rpos[0][2]=(" 
              << fitqun1r_.fq1rpos[0][2][0] << ", " << fitqun1r_.fq1rpos[0][2][1] << ", " << fitqun1r_.fq1rpos[0][2][2] << ")" << std::endl;
    std::cout << "[##### NtupleMaker::ReadDataEvent #####]  Reco direction :(" 
              << fitqun1r_.fq1rdir[0][2][0] << ", " << fitqun1r_.fq1rdir[0][2][1] << ", " << fitqun1r_.fq1rdir[0][2][2] << ")" << std::endl;
    //fiTQun variables
    fitqun_n50 = *(fitqun1r_.fqn50);
    for (int dim=0; dim<3; dim++) {
    ...
    // Reconstruction quality
    //double bsovaq = (bsgoodness*bsgoodness)-(bsdirks*bsdirks); //        <--------------------------- bonsai (try comment out)
    ...
    ncg.effwall  = effwall;
    //ncg.ovaq     = bsovaq;            <--------------------------- bonsai(try comment out)
    std::cout << "[##### NtupleMaker::ReadDataEvent #####]  Call Cherenkov(fitqun_vertex, true)" << std::endl;
    CherenkovAngle(fitqun_vertex, true);  //CherenkovAngle(bonsai_vertex, true);  //        <--------------------------- bonsai -> fiTQun
    std::cout << "[##### NtupleMaker::ReadDataEvent #####]  Finish Cherenkov(fitqun_vertex, true)" << std::endl;
    std::cout << "[##### NtupleMaker::ReadDataEvent #####]  Call Cherenkov(fitqun_vertex, false)" << std::endl;
    CherenkovAngle(fitqun_vertex, false); //CherenkovAngle(bonsai_vertex, false); //        <--------------------------- bonsai -> fiTQun
    std::cout << "[##### NtupleMaker::ReadDataEvent #####]  Finish Cherenkov(fitqun_vertex, false)" << std::endl;
    //ncg.goodness = bsgoodness;            //        <--------------------------- bonsai(try comment out)

    // search decay-e for post-activity cut
    int muerr = 1;
    float vertex_mue[ 3 ];
     vertex_mue[0] = (float)fitqun_vertex[1];  //vertex_mue[0] = (float)bonsai_vertex[ 0 ]; //        <--------------------------- bonsai -> fiTQun
     vertex_mue[1] = (float)fitqun_vertex[2];  //vertex_mue[1] = (float)bonsai_vertex[ 1 ]; //        <--------------------------- bonsai -> fiTQun
     vertex_mue[2] = (float)fitqun_vertex[3];  //vertex_mue[2] = (float)bonsai_vertex[ 2 ]; //        <--------------------------- bonsai -> fiTQun
    muechk_(vertex_mue, &muerr);
    ncg.nmue = apmue_.apnmue; 
    //------------------------------------------   fiTQun   -------------------------------------
    ...
    // fiacob - save hits for successive processing
    int icabbit=0xffff;
    ncg.nqiskz = 0;
    for ( int i = 0; i < TQREAL->nhits; i++ ) {
        ...
        if ( realcable <= MAXPM && !checkMiss( realcable ) ) {
            //if ( icflag & 1 ) {
                ncg.tiskz[ncg.nqiskz] = TQREAL->T[i];
                ncg.qiskz[ncg.nqiskz] = TQREAL->Q[i];
                ncg.icabiz[ncg.nqiskz] = realcable;
                ++ncg.nqiskz;
            //}
        }
    }
...
void NtupleMaker::CherenkovAngle(double *pvx, bool limithits) {
  ...
  if (!isMC){
    ...
    const int ALLHITS = goodcount; //ALLHITS = sktqz_.nqiskz for MC
    ...
    if (limithits)   {
        N15 = 183;
        std::cout << "  [##### NtupleMaker::CherenkovAngle #####]  tiskz=" << tiskz << ", ALLHITS=" << ALLHITS << ", N15=" << N15 << ", N15index=" << N15index << std::endl;
        std::cout << "  [##### NtupleMaker::CherenkovAngle #####]  Call MinimumWidthWindow(tiskz, ALLHITS, 15., N15, N15index)" << std::endl;
        MinimumWidthWindow(tiskz, ALLHITS, 15., N15, N15index);
        std::cout << "  [##### NtupleMaker::CherenkovAngle #####]  Finish MinimumWidthWindow(tiskz, ALLHITS, 15., N15, N15index)" << std::endl;
    }
    else {
        std::cout << "  [##### NtupleMaker::CherenkovAngle #####]  tiskz=" << tiskz << ", ALLHITS=" << ALLHITS << ", N15=" << N15 << ", N15index=" << N15index << std::endl;
        std::cout << "  [##### NtupleMaker::CherenkovAngle #####]  Call MaxHitsInWindow(tiskz, ALLHITS, 15., N15, N15index)" << std::endl;
        MaxHitsInWindow(tiskz, ALLHITS, 15., N15, N15index);
        std::cout << "  [##### NtupleMaker::CherenkovAngle #####]  Finish MaxHitsInWindow(tiskz, ALLHITS, 15., N15, N15index)" << std::endl;
    }

    std::cout << "  [##### NtupleMaker::CherenkovAngle #####]  N15=" << N15 << std::endl;
    //int maxncmb = N15*N15*N15 / 6;
    unsigned int maxncmb = N15*N15*N15 / 6;
    std::cout << "  [##### NtupleMaker::CherenkovAngle #####]  maxncmb=N15^3/6=" << maxncmb << std::endl;
    vector<double> abc2(N15*N15, 0.0); 
    vector<double> maxcmb(maxncmb, 0.0);


    //making unit direction vector form vertex to selected tubes
    if (verbose) printf("N15 is %d at index %d.\n", N15, N15index);

    //zero histogram
    hcheren->Reset();
    std::cout << "  [##### NtupleMaker::CherenkovAngle #####]  TH1D hcheren is Reset" << std::endl;
    ...
    for (int ii=N15index; ii < N15index+N15; ii++){
        ...
        //std::cout << "  [##### NtupleMaker::CherenkovAngle #####]  dtmp=" << dtmp << std::endl;
    }
    ...
    std::cout << "  [##### NtupleMaker::CherenkovAngle #####]  Finish TH1D hcheren Filling" << std::endl;
    char xtitle[64];
    ...
    std::cout << "  [##### NtupleMaker::CherenkovAngle #####]  Cherenkov angle is " << peakangle << std::endl;
    if (verbose) printf("[##### NtupleMaker::CherenkovAngle #####]  Cherenkov angle is %f\n",  peakangle);

    //hcheren->Draw();
  
    double linear_like = GetLinearLikelihood(hcheren);
    std::cout << "  [##### NtupleMaker::CherenkovAngle #####]  likelihood value is " << linear_like << std::endl;
    if (verbose) printf("[##### NtupleMaker::CherenkovAngle #####]  likelihood value is %f.\n", linear_like);
...
double NtupleMaker::GetLinearLikelihood(TH1D *hcheren) {
    ...
    printf("    [##### NtupleMaker::GetLinearLikelihood #####]  ***** FIT IS VALID? ***** = %d\n", ncg.validfit);
    ...
    logm= sumlikeli;
    delete pdfFcn;
    std::cout << "    [##### NtupleMaker::GetLinearLikelihood #####]  Finish GetLinearLikelihood()" << std::endl;
...
void NtupleMaker::MaxHitsInWindow(Float_t *t, int ALLHITS, float width, int &maxnhits, int &maxindex) {
    ...
    std::cout << "    [##### NtupleMaker::MaxHitsInWindow #####]  t=" << t << ", in:0-" << ALLHITS << ", width=" << width << std::endl;
    std::cout << "    [##### NtupleMaker::MaxHitsInWindow #####]  Call NtupleMaker::getNhits(t, in, width, ALLHITS)" << std::endl;
    for (int in=0; in<ALLHITS; in++) {
        ...
        //std::cout << "    [##### NtupleMaker::MaxHitsInWindow #####]  Hits in 15 nsec window=" << N << ", N15=" << maxnhits << std::endl;
        //If calculated hits in 15 nsec window is larger than N15, substitute it into N15.
        if (N>maxnhits) {
            maxnhits = N;
            maxindex = in;
            //std::cout << "    [##### NtupleMaker::MaxHitsInWindow #####]  Substitution: N->N15" << std::endl;
        }
    }//ALLHITS loop
    std::cout << "    [##### NtupleMaker::MaxHitsInWindow #####]  Finish NtupleMaker::getNhits(t, in, width, ALLHITS)" << std::endl;

    if (verbose) cout << "MaxHitsInWindow, maxnhits=" << maxnhits << ", maxindex=" << maxindex << endl;
}//MaxHitsInWindow()
...
void NtupleMaker::MinimumWidthWindow(Float_t *t, int ALLHITS, float width, int &maxnhits, int &maxindex) {
    ...
    std::cout << "    [##### NtupleMaker::MinimumWidthWindow #####]  t=" << t << ", ALLHITS=" << ALLHITS << ", width=" << width << ", maxnhits=" << maxnhits << ", maxindex=" << maxindex << std::endl;
    std::cout << "    [##### NtupleMaker::MinimumWidthWindow #####]  Call MaxHitsInWindow(t, ALLHITS, width, maxnhits, maxindex)" << std::endl;
    double dt   = 100000.;
    ...
    if (dt>width) {
        if (verbose) cout << "dt too wide: ";
        MaxHitsInWindow(t, ALLHITS, width, maxnhits, maxindex);
    }
    else {
        if (verbose) cout << "MinimumWidthWindow, maxnhits=" << maxnhits << ", maxindex=" << maxindex << endl;
    }
    std::cout << "    [##### NtupleMaker::MinimumWidthWindow #####]  Finish MaxHitsInWindow(t, ALLHITS, width, maxnhits, maxindex)" << std::endl;
