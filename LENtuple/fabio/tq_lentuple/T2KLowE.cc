...
void T2KLowE::createTree (TTree *tree) {
    ...
    tree->Branch("dt0",                &dt0,                "dt0/F");         //<---------------- (try comment out)
    tree->Branch("dt0vtx",             &dt0vtx,             "dt0vtx/F");      //<---------------- (try comment out)
    tree->Branch("dt0type",            &dt0type,            "dt0type/I");     //<---------------- (try comment out)
    tree->Branch("n30max",             &n30max,             "n30max/I");      //<---------------- (try comment out)
    tree->Branch("n30time",            &n30time,            "n30time/F");     //<---------------- (try comment out)
    tree->Branch("potot",              &potot,              "potot/F");     //ID totpe (qismsk)
    tree->Branch("nhit",               &nhit,               "nhit/I");     //ID hits (nqisk)
    tree->Branch("n50",                &n50,                "n50/I");     // N50 from Bonasi fit      <------------------------ bonsai

    tree->Branch("pos",                pos,                 "pos[3]/F");    // Bonsai fit vertex      <------------------------ bonsai
    tree->Branch("bdir",               bdir,                "bdir[3]/F");
    tree->Branch("erec",               &erec,               "erec/F");      // Bonsai fit energy      <------------------------ bonsai(try comment out)
    tree->Branch("wall",               &wall,               "wall/F");
    tree->Branch("effwall",            &effwall,            "effwall/F");
    tree->Branch("ovaq",               &ovaq,               "ovaq/F");                             // <------------------------ bonsai(try comment out)
    ...
    //tree->Branch("goodness",           &goodness,           "goodness/F");  // Bonsai fit goodness    <------------------------ bonsai(try comment out)
    ...
    // Primary selection for MC and DATA
    tree->Branch( "trgofst", &trgofst, "trgofst/F" );
    tree->Branch( "nqiskz",            &nqiskz,       "nqiskz/I" );
    tree->Branch("icabiz",             icabiz,       "icabiz[nqiskz]/I");  // ID cables (icabiz)
    tree->Branch("tiskz",              tiskz,        "tiskz[nqiskz]/F");  // ID time (tiskz)
    tree->Branch("qiskz",              qiskz,        "qiskz[nqiskz]/F");  // ID charge (qiskz)
 
    tree->Branch( "validfit", &validfit, "validfit/I" );
    tree->Branch( "tbgn", &tbgn, "tbgn/F" );
    tree->Branch( "tend", &tend, "tend/F" );
    tree->Branch( "tdur", &tdur, "tdur/F" );
  
    if ( Is_mc ) {
        // Primary selection for MC only
